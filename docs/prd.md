...
Necesita definir los campos de una plantilla de formulario, crear formularios a partir de ella y compartirlos para que otras personas los rellenen. Un caso de uso central es el de un product owner que, dado un requisito, define mediante una `FormTemplate` qué datos necesita dejar claros para que la tarea pueda pasar a la fase de desarrollo.
...
- Crear plantillas de formulario definiendo sus campos y tipos de dato una única vez.
- Crear formularios a partir de una plantilla existente, listos para compartir sin tener que redefinir sus campos.
...
### MVP (versión inicial)
- Creación, edición y eliminación de plantillas de formulario, incluyendo la configuración de sus campos (tipo de dato, obligatoriedad, opciones, orden).
- Creación, edición y eliminación de formularios a partir de una plantilla de formulario existente, sin necesidad de volver a definir sus campos.
...
- Control de acceso o restricciones de visibilidad sobre plantillas de formulario, formularios o respuestas.
...
- **Backend**: expone la API necesaria para la gestión de todos los datos del dominio (plantillas de formulario, campos de formulario, formularios, respuestas), protegida por una contraseña compartida simple, sin autenticación real de usuarios ([docs/backend.md](backend.md) §2 y §5).
...
### 8.1 Creación y gestión de plantillas de formulario y formularios
- Como usuario, quiero crear una plantilla de formulario dándole un nombre y una descripción, para definir una estructura de datos reutilizable.
- Como usuario, quiero añadir, editar, reordenar y eliminar campos de una plantilla de formulario, eligiendo su tipo de dato y si son obligatorios, para definir la estructura de captura que necesito.
- Como usuario, quiero crear un formulario a partir de una plantilla de formulario existente, para compartirlo sin tener que redefinir sus campos cada vez.
- Como usuario, quiero eliminar una plantilla de formulario o un formulario que ya no necesito, para mantener ordenados los listados.
...
- Una plantilla de formulario (`FormTemplate`) se compone de una lista ordenada de campos configurables (`FormTemplateField`), cada uno con su propio tipo de dato y obligatoriedad.
- Un formulario (`Form`) se crea a partir de una `FormTemplate` copiando sus campos como `FormField` propios en ese momento: varios formularios pueden partir de la misma plantilla sin redefinir la estructura, y cada uno queda después desacoplado de ella (editar o eliminar la plantilla no afecta a los formularios ya creados).
...
- No hay restricciones de acceso: cualquiera puede crear, editar o eliminar cualquier plantilla de formulario o formulario, y rellenar, editar o consultar la respuesta de cualquier formulario.

### 9.2 Comportamiento esperado
1. Se crea una plantilla de formulario y se definen sus campos.
2. Se crea uno o varios formularios a partir de esa plantilla, listos para compartir; cada uno copia la estructura de campos en ese momento y ya no cambia si la plantilla cambia después.
...
1. Creación, edición y eliminación de plantillas de formulario.
2. Configuración de campos de una plantilla de formulario (tipo, obligatoriedad, opciones, orden).
3. Creación, edición y eliminación de formularios a partir de una plantilla de formulario.
...
- La solución puede evolucionar además hacia lógica condicional entre campos, sincronización de un formulario con cambios posteriores en su plantilla de origen, o exportación de datos en fases posteriores, si la necesidad de esas features avanzadas lo justifica.
